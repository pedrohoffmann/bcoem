...
//filtro de CPF
	if ($filter != "admin") {
			...
			$query_cpf = "SELECT cpf FROM $database.cpf WHERE cpf = '$brewerCPF'";
			$checar_cpf = mysqli_query($connection,$query_cpf) or die (mysqli_error($connection));
			$row_checar_cpf = mysqli_fetch_assoc($checar_cpf);
			$totalRows_checar_cpf = mysqli_num_rows($checar_cpf);
			if ($totalRows_checar_cpf > 0){
				$cpf_existe = TRUE;
			}
	...
	if (($view == "default") && ($filter != "admin") && (!$captcha_success)) {
		...
		setcookie("brewerCPF", $brewerCPF, 0, "/");
	...
	elseif (($view == "default") && ($filter != "admin") && ($cpf_existe == FALSE)) {
		...
		if ($filter == "admin") $location =  $base_url."index.php?section=admin&go=entrant&action=register&msg=227";
		else $location = $base_url."index.php?section=".$section."&go=".$go."&msg=227";
		header(sprintf("Location: %s", $location));
		...
		if ($totalRows_userCheck > 0) {
			...
			if (($brewerAHA < "999999994") || ($brewerAHA == "")) {
				...
				%s
							   ...
							   GetSQLValueString($brewerCPF, "text")
							   );
			...
			if ($_SESSION['prefsEmailRegConfirm'] == 1) {
				...
				if ($show_entrant_fields) {
					...
					//if (isset($_POST['brewerCPF'])) $message .= sprintf("<tr><td valign='top'><strong>%s:</strong></td><td valign='top'>%s</td></tr>",$label_steward,$brewerCPF);
